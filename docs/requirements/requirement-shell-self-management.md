...
This requirement is the **project Single Source of Truth** for **CLI self-management** of the nginx-config POSIX shell tool: inspecting, upgrading, and removing its own installed binary (and related install artifacts) safely—especially for tools installed via one-command online install (`curl | sh`)—without requiring a separate package-manager workflow for routine updates.
...
**In one sentence:** You can check, update, or remove **this program file** yourself — that does not apply nginx host conf.

| Box | Meaning | Example |
|-----|---------|---------|
| You / this login | `version-check` / `self-update` / `self-uninstall` / `about` as yourself | `nginx-config self-update` |
...
| Includes | Excludes |
|----------|----------|
| `version-check`, `self-update`, `self-uninstall`, `about` | host nginx packages |
| Reuse of install primitives | Empty argv = help |

| Surface | What you open | What for |
|---------|---------------|----------|
| `nginx-config about` | command | install + folder diagnostics |
| `nginx-config self-uninstall` | command | remove CLI binary |

| You do… | What it means | What you type |
|---------|---------------|---------------|
| Update the CLI | Fetches the channel, verifies digest, replaces the installed binary. Saved profiles stay. | `nginx-config self-update` |
...
| **Repository identity** | `REPO_USER` default `cloudgen`; `REPO_NAME` default `nginx-conf` |
| **Release channel** | `SCRIPT_URL` Config default composed as `https://raw.githubusercontent.com/${REPO_USER}/${REPO_NAME}/main/${APP_NAME}` (this project: `https://raw.githubusercontent.com/cloudgen/nginx-conf/main/nginx-config` — product channel SSOT; override `SCRIPT_URL` or `REPO_*` via env if needed) |
...
| `SCRIPT_URL` default channel URL | **This project:** non-empty product default composed from `REPO_USER` / `REPO_NAME` / `APP_NAME` (`https://raw.githubusercontent.com/cloudgen/nginx-conf/main/nginx-config`); product README must show simple literal one-liner(s) from that SSOT; env may still override |
...
Work claiming self-management support for nginx-config is **not done** if any of the following fail:
